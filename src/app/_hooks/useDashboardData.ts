...
function combinedEntries(calsync: FoodEntry[], dropsync: DrinkEntry[]): RecentEntry[] {
	const food: RecentEntry[] = calsync.map(e => ({
		type: 'food',
		name: e.food || 'Food',
		icon: e.emoji || 'fa-solid fa-utensils',
		color: e.color || 'var(--accent)',
		amount: Math.round(e.kcal || 0) + ' kcal',
		meta: formatTime(e.ts),
		ts: e.ts || 0,
		date: entryDate(e),
	}));
	const drinks: RecentEntry[] = dropsync.map(e => ({
		type: 'drink',
		name: e.drink || 'Drink',
		icon: e.emoji || 'fa-solid fa-droplet',
		color: e.color || '#5cc9fa',
		amount: '+' + formatMl(e.amount || 0),
		meta: formatTime(e.ts),
		ts: e.ts || 0,
		date: entryDate(e),
	}));

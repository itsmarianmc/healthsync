...
export default function CalSyncModal({ isOpen, onClose, onLog, onShowToast, openWithAi, openWithBarcodeValue }: CalSyncModalProps) {
  ...
  const handleImageFile = useCallback(async (file: File) => {
    ...
    } catch (e) { handleAIError(e as Error); }
  ...
  const handleCameraFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      populateAIResult(result);
    } catch (e) { handleAIError(e as Error); console.error(e); }
  }, [populateAIResult, handleAIError, onShowToast]);
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (!modalRef.current) return;
    if (modalState === 'expanded') { if (dy > 0) modalRef.current.style.height = Math.max(naturalH.current, expandedH() - dy) + 'px'; }
    else { if (dy > 0) modalRef.current.style.transform = `translateY(${dy}px)`; else { modalRef.current.style.height = Math.min(expandedH(), naturalH.current + Math.abs(dy)) + 'px'; modalRef.current.style.transform = 'translateY(0)'; } }
  };
  ...
  const runSearch = async (q: string) => {
    ...
    } catch { setSearchLoading(false); setSearchResults([]); setSearchStatus('Search failed. Check your connection.'); }
  };
  ...
  return (
    <>
      ...
      <div className={`modal${aiMethodOpen ? ' small' : ''}`} id="modal" ref={modalRef}>
        <div className="modal-handle-zone" id="handleZone"
            ...
            <div className="modal-step" id="cs-step1" ref={step1Ref} style={{ display: 'block', width: '25%', flexShrink: 0 }}>
              <div className="search-step-inner">
                <div className="category-grid" id="categoryGrid">
                  {CATEGORIES.map(cat => (
                    <div key={cat.category} className={`category-option${category?.category === cat.category ? ' selected' : ''}`}
                      data-category={cat.category} data-emoji={cat.emoji} data-color={cat.color}
                      onClick={() => { setCategory(cat); setSelFood(null); goToStep(2); }}>
            ...
            <div className="modal-step" id="cs-step2" ref={step2Ref} style={{ display: 'block', width: '25%', flexShrink: 0 }}>
              <div className="search-step-inner">
                ...
                <div className="ai-processing" id="aiProcessing" style={{ display: aiProcessing ? 'block' : 'none' }}>
                  <div className="processing-spinner"><i className="fa-solid fa-circle-notch fa-spin" /></div>
                        ...
                        <>
                          ...
                          {favs.slice(0, 5).map((f, i) => (
                            <div key={i} className="recent-item fav-item" onClick={() => { setShowRecent(false); selectFood(f); }}>
                              <div className="recent-item-icon"><i className={f.emoji || 'fa-solid fa-utensils'} style={{ color: f.color || 'var(--accent)' }} /></div>
                              <div className="recent-item-info">
            ...
            {selFood && (
              <div className="food-confirm-inner">
                ...
                <div className="calorie-preview-row" id="caloriePreviewRow">
                  <div className="calorie-big"><span id="calculatedCalories">{Math.round(calcKcal())}</span><span className="calorie-unit-label">kcal</span></div>
        ...
        <div className="modal-footer" id="modalFooter">
          <button className="confirm-btn" id="actionBtn" disabled={!isLastStep || !selFood}
            onClick={() => { if (isLastStep && selFood) logFood(); }}>
